 ...
 * to the memory area pointed to by `destination`. The memory areas should not
 * overlap.
 *
...
/**
 * @brief Sets the first `n` bytes of the block of memory pointed by `pointer`
 ...
 * `pointer1` and `pointer2`. The comparison is done byte by byte (unsigned
 * char), and stops as soon as a differing byte is found.
 *
 ...
 * differing byte in `pointer1` is found, respectively, to be less than, equal
 * to, or greater than the corresponding byte in `pointer2`. If the two memory
 * blocks are identical for the first `n` bytes, the function returns 0.
 */
