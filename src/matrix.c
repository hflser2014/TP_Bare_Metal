...
void matrix_init() {
  ...
  GPIOA->MODER &=
      ~(GPIO_MODER_MODE2_1 | GPIO_MODER_MODE3_1 | GPIO_MODER_MODE4_1 |
  ...
  GPIOB->MODER &=
      ~(GPIO_MODER_MODE0_1 | GPIO_MODER_MODE1_1 | GPIO_MODER_MODE2_1);
  GPIOB->MODER |=
  ...
  GPIOA->OSPEEDR |=
      (GPIO_OSPEEDR_OSPEED2 | GPIO_OSPEEDR_OSPEED3 | GPIO_OSPEEDR_OSPEED4 |
       GPIO_OSPEEDR_OSPEED5 | GPIO_OSPEEDR_OSPEED6 | GPIO_OSPEEDR_OSPEED7 |
       GPIO_OSPEEDR_OSPEED15);
  GPIOB->OSPEEDR |=
      (GPIO_OSPEEDR_OSPEED0 | GPIO_OSPEEDR_OSPEED1 | GPIO_OSPEEDR_OSPEED2);
  GPIOC->OSPEEDR |=
...
void pause(int time) {
  // Wait time in ns
  // We suppose the system clock is 40MHz
  ...
  for (int i = 0; i < 8; i++) {
    ...
    for (unsigned int j = 0; j < 8; j++) {
      ...
      val[j].r = color_order[0] == 0   ? base_value * (steps - t) / steps
                 : color_order[0] == 1 ? base_value * t / steps
                                       : 0;
      val[j].g = color_order[1] == 0   ? base_value * (steps - t) / steps
                 : color_order[1] == 1 ? base_value * t / steps
                                       : 0;
      val[j].b = color_order[2] == 0   ? base_value * (steps - t) / steps
                 : color_order[2] == 1 ? base_value * t / steps
                                       : 0;
    }
    mat_set_row(i, val);
  }
...
void display_static_image(const uint8_t* image_start, const int image_size) {
  ...
  int size_in_row = image_size / sizeof(rgb_color) /
                    8;  // calculate the size of the image in row
