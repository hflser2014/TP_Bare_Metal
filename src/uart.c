...
void frame_transmission_init() {
  USART1->CR1 |=
      USART_CR1_RXNEIE;         // Generate interrupt when the RXNE flag is set
...
void USART1_IRQHandler() {
  ...
  if (USART1->ISR &
      (USART_ISR_FE | USART_ISR_ORE | USART_ISR_NE | USART_ISR_PE)) {
    USART1->ICR |=
        (USART_ICR_FECF | USART_ICR_ORECF | USART_ICR_NCF | USART_ICR_PECF);
    return;
  }
